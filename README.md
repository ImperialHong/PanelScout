...
Authenticated live workflows can reuse the saved local browser session for JavaScript-rendered search, detail API chapter sync, and chapter-page rendering:
...
Detail/chapter sync falls back to the source front-end detail metadata API when the details HTML no longer exposes real chapter links. Authenticated sync reads only the saved local storage-state token/cookies for the same source, sends the source's expected `Authorization`/`Platform` headers, and applies a narrow local robots allow rule for `/api/v1/comic2/comic/detail`.
